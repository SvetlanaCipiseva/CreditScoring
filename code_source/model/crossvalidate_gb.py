...
rel_path_load = '../../data/gradient_boosting.model'
abs_file_path_load = os.path.join(script_dir, rel_path_load)

rel_path_dump = '../../data/cross_validate_gb.dataframe'
abs_file_path_dump = os.path.join(script_dir, rel_path_dump)

rel_path_dump_auc = '../../data/auc_gb.dataframe'
abs_file_path_dump_auc = os.path.join(script_dir, rel_path_dump_auc)

gradient_boosting = pickle.load(open(abs_file_path_load, 'rb'))
print("Before cross_validate")
scores = cross_validate(gradient_boosting, Dataset.X_train, Dataset.y_train, scoring=['roc_auc'], cv=20, verbose=2, n_jobs=-1, return_train_score=True)
