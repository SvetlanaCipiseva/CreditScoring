...
rel_path_load = '../../data/svm_poly.model'
abs_file_path_load = os.path.join(script_dir, rel_path_load)

rel_path_dump = '../../data/cross_validate_svm_poly.dataframe'
abs_file_path_dump = os.path.join(script_dir, rel_path_dump)

rel_path_dump_auc = '../../data/auc_svm_poly.dataframe'
abs_file_path_dump_auc = os.path.join(script_dir, rel_path_dump_auc)

svm_poly = pickle.load(open(abs_file_path_load, 'rb'))
print("Before cross_validate")
scores = cross_validate(svm_poly, Dataset.X_train_scaled, Dataset.y_train, scoring=['roc_auc'], cv=20, verbose=2, n_jobs=-1, return_train_score=True)
